...
void Level2::updateLevelSpriteState()
{
	vector<Zombie*>::iterator p;
	for (p = zombies.begin(); p != zombies.end(); p++)
	{
...
void Level2::mouseMove(int x, int y)
{
...
void Level2::mouseClick(int x, int y)
{
...
int Level2::getClickBlock(int x, int y)
{
	int Index = -1;
	POINT pt;
	pt.x = x;
	...
	int iCount = 0;
	for (int i = 0; i < LEVEL2_VALID_BLOCKNUMBER; i++)
	{
		rec.left = LEVEL2_VALID_BLOCKPOS[i][0];
		rec.top = LEVEL2_VALID_BLOCKPOS[i][1];
		rec.right = rec.left + LEVEL2_VALID_BLOCKPOS[i][2];
		rec.bottom = rec.top + LEVEL2_VALID_BLOCKPOS[i][3];
		if (PtInRect(&rec, pt))
		{
			return iCount;
		}
...
void Level2::initSound(HWND m_hWnd)
{
...
void Level2::handlleSpriteDead()
{
	handlePlantDead();
	handleZombieDead();
}

void Level2::handlePlantDead()
{
	//处理植物的死亡
...
void Level2::addZombie(int zomeType, int num)
{
	for (int i = 0; i < num; i++)
	{
		int type = rand() % 3;
...
void Level2::Draw(HDC hdc, int x, int y)
{
	// 计算场景当前坐标与场景上一次坐标的差值
	int offsetX = 0;
	int offsetY = 0;
